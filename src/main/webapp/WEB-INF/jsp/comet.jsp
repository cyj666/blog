...
<body>
	Time：
	<span id="kb">...</span>
	聊天内容<p id="qq" style="border:solid black 1px;">...</p>
...
var c = null;
	function init() {
		...
		var kbDom1 = document.getElementById('qq');
		...
		JS.Engine.on({
			...
			ok : function(kb) {//侦听一个channel
				kbDom1.innerHTML = kb;
			},
				
			start : function(cId, channelList, engine){
		      alert('连接已建立，连接ID为：' + cId + " ,channeList:"+ channelList +",engine:"+engine);
		      c = cId;
		      $.ajax({
				    url:'start?cId='+cId+"&&name=a",
				    type:'GET', //GET
		    ...
		    stop : function(cause, cId, url, engine){
		      alert('连接已断开，连接ID为：' + cId + ',断开原因：' + cause + ',断开的连接地址：'+ url+",engine:"+engine);
		    }
		});
	...
	$('.submit').click(function(){
		//alert($(':text').val());
		$.ajax({
			 url:'test?value='+$(':text').val()+"&&myName=a"+"&&toName=b",
		    type:'GET', //GET
		    async:true,    //或false,是否异步
		    //data:{value=$(':text').val()},
		    processData : false, //告诉jQuery不要处理发送的数据
